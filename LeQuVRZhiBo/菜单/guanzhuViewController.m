...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    FansListData*data=dataSource[indexPath.row];
    person.uid = data.dataIdentifier;
...
- (NSArray *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //添加一个更多按钮
    FansListData*data=dataSource[indexPath.row];
    UITableViewRowAction *moreRowAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleNormal title:@"取消关注" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        NSLog(@"取消关注");
        [self cancleConcern:data.dataIdentifier];
    }];
...
- (void)getFansList
{
    [AFManager getReqURL: [NSString stringWithFormat:GUANZHULIST,[USERDEFAULT valueForKey:@"uid"],page] block:^(id infor) {
...
{
    [AFManager getReqURL:[NSString stringWithFormat:DELGUANZHU,[USERDEFAULT valueForKey:@"uid"],ID] block:^(id infor) {
        if ([[NSString stringWithFormat:@"%@",infor[@"code"]] isEqualToString:@"200"])
        {
