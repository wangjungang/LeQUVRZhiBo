    ...
    for (int i = beginEmojiBtn; i < endEmojiBtn; i++) {
       ...
       // NSLog(@"<string>%@</string>", faceEmojiArray[i]);
        ...
        if (((i + 1) >= self.emojiMaxCountPerPage && (i + 1) % self.emojiMaxCountPerPage == 0) ||
            ...
            [deleteButton setImage:[UIImage imageNamed:@"emoji_btn_delete.png"] forState:UIControlStateNormal];
...
-(void)loadCustomerEmotionPackage{
    ...
    [emojiBtn setImage:[UIImage imageNamed:@"emoji_btn_normal.png"] forState:UIControlStateNormal];
