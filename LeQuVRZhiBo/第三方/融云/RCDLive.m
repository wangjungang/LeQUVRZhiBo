...
- (void)getUserInfoWithUserId:(NSString *)userId completion:(void (^)(RCUserInfo *))completion{
    ...
    [manager POST:TOKEN parameters:@{@"uid":userId} constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if ([responseObject[@"result"] isEqualToString:@"1"]) {
            ...
            user.userId = userId;
            user.name = responseObject[@"info"][@"nickname"];
            user.portraitUri = responseObject[@"info"][@"imgUrl"];
