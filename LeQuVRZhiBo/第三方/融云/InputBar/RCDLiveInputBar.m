...
/*!
 当前输入框状态
 */
@property(nonatomic) KBottomBarStatuss currentBottomBarStatus;
 ...
 *  @param Status
 */
-(void)setInputBarStatus:(KBottomBarStatuss)Status{
...
-(void)changeInputBarFrame:(CGRect)frame{
    self.originalFrame = frame;
    [self setFrame:frame];
    [self setInputBarStatus:KBottomBarDefaultStatuss];
...
- (void)KeyboardWillHide:(NSNotification*)notification {
    ...
    [self animationLayoutBottomBarWithStatus:KBottomBarDefaultStatuss animated:YES];
    self.currentFrame = self.frame;
    self.KeyboardFrame = CGRectZero;
    self.currentInputBarHeight = self.frame.size.height;
    [self chatSessionInputBarControlContentSizeChanged:self.chatSessionInputBarControl.frame];
...
- (void)didTouchSwitchButton:(BOOL)switched {
    ...
    if (switched) {
        [self animationLayoutBottomBarWithStatus:KBottomBarDefaultStatuss animated:YES];
        if (_currentBottomBarStatus != KBottomBarDefaultStatuss) {
            [self.chatSessionInputBarControl.inputTextView resignFirstResponder];
        }
...
- (void)didTouchEmojiButton:(UIButton *)sender {
    ...
    if (_isClickEmojiButton) {
        [self.chatSessionInputBarControl.inputTextView becomeFirstResponder];
        [sender setImage:[UIImage imageNamed:@"chatting_biaoqing_btn_normal.png"] forState:UIControlStateNormal];
        _isClickEmojiButton = NO;
        self.chatSessionInputBarControl.inputTextView.inputView = nil;
    }else{
        _isClickEmojiButton = YES;
        [sender setImage:[UIImage imageNamed:@"chat_setmode_key_btn_normal.png"] forState:UIControlStateNormal];
...
- (void)animationLayoutBottomBarWithStatus:(KBottomBarStatuss)bottomBarStatus animated:(BOOL)animated{
    if (bottomBarStatus == KBottomBarDefaultStatuss) {
        _isClickEmojiButton = NO;
        _isClickAddButton = NO;
    }
    if (bottomBarStatus != KBottomBarEmojiStatuss) {
        self.chatSessionInputBarControl.inputTextView.inputView = nil;
    ...
    if (bottomBarStatus == KBottomBarEmojiStatuss && !_emojiBoardView) {
        [self emojiBoardView];
    }
    
    if (bottomBarStatus == KBottomBarKeyboardStatusss) {
        [self.chatSessionInputBarControl.inputTextView becomeFirstResponder];
    }
    if (animated == YES) {
...
- (void)layoutBottomBarWithStatus:(KBottomBarStatuss)bottomBarStatus {
    if (bottomBarStatus != KBottomBarKeyboardStatusss) {
    ...
    switch (bottomBarStatus) {
        ...
        case KBottomBarKeyboardStatusss: {
            CGRect frame = self.originalFrame;
            frame.origin.y = frame.origin.y - self.chatSessionInputBarControl.frame.size.height + Height_ChatSessionInputBar;
            frame.size.height += chatInputBarRect.size.height - Height_ChatSessionInputBar;
            [self setFrame:frame];
