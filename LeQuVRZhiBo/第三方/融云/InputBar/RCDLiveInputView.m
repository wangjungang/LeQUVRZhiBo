...
- (void)resetInputBar {
    ...
    if ([type isEqualToString:@"1"]) {
        ...
        [self configInputContainerView2];
    }
    
    [self setAutoLayoutForSubViews];
    [self rcInputBar_registerForNotifications];
}
-(void)switchInputBoxOrRecord:(UIButton *)btn{
    if (btn.selected == YES) {
        if (self.delegate2&&[self.delegate2 respondsToSelector:@selector(diddanmu:)]) {
            [self.delegate2 diddanmu:@"1"];
        }
        btn.selected = NO;
...
- (void)configInputContainerView {
        ...
        self.switchButton = [[UIButton alloc] initWithFrame:CGRectMake(5, 9, 52, 31)];
        [_switchButton setImage:RCDLive_IMAGE_BY_NAMED(@"关闭弹幕.png") forState:UIControlStateNormal];
        [_switchButton setImage:RCDLive_IMAGE_BY_NAMED(@"开启弹幕.png") forState:UIControlStateSelected];
        ...
        self.emojiButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width-36, 9, 31, 31)];
        [_emojiButton setImage:RCDLive_IMAGE_BY_NAMED(@"表情.png") forState:UIControlStateNormal];
        [_emojiButton setImage:RCDLive_IMAGE_BY_NAMED(@"表情.png") forState:UIControlStateSelected];
        ...
        self.inputTextView = [[myTextView alloc] initWithFrame:CGRectMake(10, 5, self.frame.size.width-10-41, 36)];
...
- (void)configInputContainerView2{
...
////                                                                   views:_bindingViews]];
//    [self.inputContainerSubViewConstraints
//     addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-10-[_inputTextView(36)]"
//                                                                 options:0
//                                                                 metrics:nil
//                                                                   views:_bindingViews]];
//    
